...
create table if not exists public.conversation (
    ...
    type varchar(32) not null default 'RAG',
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now(),
    deleted_at timestamptz,

    constraint chk_conversation_type check (type in ('RAG', 'LITERATURE'))
);
...
comment on column public.conversation.type is '会话类型：RAG、LITERATURE';
...
alter table if exists public.conversation
    add column if not exists type varchar(32) not null default 'RAG';
...
alter table if exists public.conversation
    drop constraint if exists chk_conversation_type;

alter table if exists public.conversation
    add constraint chk_conversation_type check (type in ('RAG', 'LITERATURE'));
